"""Nigeria driver's license parsing rules."""
...
def parse_nigeria_drivers_license(text: str) -> Dict[str, Any]:
    """Parse common fields from a Nigerian driver's license."""
    data = {
        "license_number": first_match(text, r"\b(?:DLN|LICENSE\s*(?:NO|NUMBER)|LICENCE\s*(?:NO|NUMBER))[:\s-]*([A-Z0-9-]{6,30})"),
        ...
        "address": first_match(text, r"\bADDRESS[:\s-]*([A-Z0-9 ,'#./-]{5,120})"),
    }
    data = {key: value for key, value in data.items() if value}
    success = bool(data.get("license_number") or data.get("full_name"))
    return {
        "success": success,
        "message": None if success else "Could not extract Nigerian driver's license data.",
        "document_type": "DRIVERS_LICENSE",
        "country": country_validation_summary(country_code="NGA", document_type="DRIVERS_LICENSE", extracted_data=data),
        "data": data,
